...
```bash
docker pull ghcr.io/bodencrouch/cloudflare-one-gui-linux:latest
docker run --rm -p 4173:4173 -e WARP_CLI=/path/to/warp-cli ghcr.io/bodencrouch/cloudflare-one-gui-linux:latest
```

CI builder image:

```bash
docker pull ghcr.io/bodencrouch/cloudflare-one-gui-linux-ci:latest
```
...
```bash
brew tap bodencrouch/cloudflare-one-gui-linux homebrew-tap
