...
test("checkForUpdate with mocked GitHub", async () => {
            ...
            assets: [
              {
                ...
                browser_download_url: "https://github.com/bodencrouch/cloudflare-one-gui-linux/releases/download/v9.9.9/thirdflare-9.9.9-x86_64.AppImage",
                size: 10,
  ...
  try {
    const result = await checkForUpdate(
      {
        updates: {
          channel: "stable",
          source: { owner: "bodencrouch", repo: "cloudflare-one-gui-linux" }
        }
      },
    ...
    assert.equal(result.recommendedAsset.url, "https://github.com/bodencrouch/cloudflare-one-gui-linux/releases/download/v9.9.9/thirdflare-9.9.9-x86_64.AppImage");
  } finally {
...
test("applyUpdate rejects client-supplied assetUrl", async () => {
  ...
  const result = await applyUpdate(
    { updates: { channel: "stable", source: { owner: "bodencrouch", repo: "cloudflare-one-gui-linux" } } },
...
test("applyUpdate requires confirmation token for AppImage", async () => {
  clearGithubCache();
  const originalFetch = globalThis.fetch;
  const assetUrl = "https://github.com/bodencrouch/cloudflare-one-gui-linux/releases/download/v9.9.9/thirdflare-9.9.9-x86_64.AppImage";
  globalThis.fetch = async (url) => {
    const href = String(url);
    if (href.includes("update-manifest.json")) {
